...
tlm_transport_channel，是一个sc_module，用来讲一个<<tlm::1::tlm_req_resp_channel, tlm_req_resp_channel>>
...
<<tlm::1::tlm_master_to_transport, tlm_master_to_transport>>的成员变量target_export上，注意，这是一个export
到export的绑定，所以实际上绑定的结果是tlm_master_to_transport的成员变量target_export先转成一个interface类型，再被
tlm_transport_channel的target_export所绑定。

接下来，tlm_master_to_transport的master_port绑定了这里req_rsp的master_export。get_request_export和put_response_export
...
与随后要介绍的<<tlm::1::tlm_save_to_transport>>才是一对。
...
TLM 1.0的Analysis部分是一系列小的interface, fifo的组合，主要用来做辅助性的功能。
...
NOTE: 为何tlm_analysis_port名字里带有port，实际上却实现为 tlm_analysis_if的子类呢？原因目前还不知晓，后续等明晰了相关设计细节
后再进行补充。
