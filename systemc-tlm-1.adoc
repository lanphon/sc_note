...
数据被get到之后，数据就等于消费掉了。而peek是一种retrieve操作，或者query操作，是查询元素。peek一个元素，并不会
...
像sc_fifo一样，使用一个raw buffer加上读写指针来模拟circular buffer。此外，tlm_fifo支持在运行时调整size，这
...
* 传输RSP的通道response_fifo，默认是tlm_fifo<RSP>类型
...
tlm_transport_to_master，从名字来看，这是将数据从transport接口实现，转到master接口实现。因为tlm_transport_to_master
