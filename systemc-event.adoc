...
wait是Thread/CThread线程用以同步的一个API调用。SystemC中定义了多种形式的wait。例如，sc_module
...
执行当前线程。wait_cycles实际上并不是等待一个时钟周期，而是等待一个静态事件而已。
...
类似于wait, next_trigger也在多个地方有定义。sc_module中的next_trigger定义有：
...
clear_trigger的代码需要结合不同的trigger type来看。例如，之前设定为AND_LIST_TIMEOUT
trigger类型，那么在clear trigger的时候，就需要将两个事件对象都做清除。
