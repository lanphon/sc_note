...
直接的sc_event的时候，调用的就是 `default_event` 返回一个事件引用
...
sc_clock是一个sc_signal<bool, SC_ONE_WRITER>的继承类，所以sc_clock可以被sc_in<bool>
绑定：
...
1. 在sc_module构造的时候，作为准备绑定sc_clock的 sc_in<bool>，如何将之设置为静态敏感源
2. 在hierarchy建立之后，仿真开始之前，如何resolve之前设置的静态敏感源，使之得到真正的event事件
...
这里将sc_event_finder先设置在sc_in<bool> clk中。
...
最终调用的也是讲sc_event_finder设置在sc_in<bool> clk中。
...
同时也会调用sc_in<bool> clk的complete_binding函数。通过给的interface指针 `&clock`
以及成员函数指针 `posedge_event`
...
在运行中，由于sc_clock的自激励，`*m_posedge_event_p` 在UPDATE PHASE，根据
primitive channel的framework, `update` 函数被调用：
...
这又调用了do_update函数。在do_update函数中，除了调用基类的do_update函数外，最为重要的
...
这等同调用 `m_posedge_event_p` 的notify_next_delta函数，此时位于crunch的UPDATE PHASE。
