...
A port
is a pointer to a channel outside the module. An export is a pointer to a channel inside
another module.
...
sc_port_registry是一个管理类，这个类管理了所有生成的sc_port_base以及继承类：当每个
sc_port_base以及继承类构造的时候，都会将自己注册到sc_port_registry中。
...
* `get_interface` 包括const类型的重载形式，用已返回一个sc_interface指针。根据其后的实现，这里
...
如果设定的是SC_ALL_BOUND，那么没有绑定，或者实际绑定的数目小于设定的绑定数目的时候，就会报
...
对于每一个继承自sc_interface的派生类而言，接口函数default_event能够返回一个event是事件。这个
