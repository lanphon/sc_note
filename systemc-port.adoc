...
sc_port_base是一个继承自<<data::sc_object, sc_object>>的接口类，并且不是一个模板类，所以sc_port_base是不包括
...
为了能让sc_port作为static sensitive的源，这里使用了<<api::sc_event_finder, sc_event_finder>>来作为占位符，将
...
sc_port_base的make_sensitive只是将需要绑定的handle和<<api::sc_event_finder, sc_event_finder>>放入一个
...
主要调用了<<api::sc_event_finder, sc_event_finder>>部分的功能，我们在event_finder
...
((sc_port))继承自sc_port_b<IF>，同时增加了更多的模板参数和功能：
[[api::sc_port]]
...
((sc_in))是signal类型的输入端口模型，需要绑定到signal类型上，不能绑定到fifo类型上。
...
介绍sc_out之前，我们先介绍一下((sc_inout))类型。sc_inout并不像verilog HDL那种inout类型一样，
...
((sc_out))类型其实就是一个sc_inout类型。SystemC在代码里给出了定义一个新的模板类，而不是
...
.不使用sc_export做binding的场景
image::bind_without_export.png[]
...
缓冲数据。为了隐藏这种实现细节，SystemC从版本2.1开始，提供了一个新的模板类， `sc_export<T>` 。
使用sc_export做绑定的示例如下：

.不使用sc_export做binding的场景
image::bind_with_export.png[]

使用((sc_export))的应用场景和上边的第二种情况差不多。所不同的是，此时PB声明自己有一个public的
...
此时，sc_port表示一个模块**需要**的interface，而一个sc_export则表示一个模块**提供**的interface。
sc_port indicates sc_interface is **required** by a module, and sc_export
indicates **provided** by a module.
...
sc_export_base是所有sc_export<IF>的基类，而sc_export_base则直接继承自
sc_object，是一个纯虚的基类：
...
sc_export_base没有定义任何成员变量，这意味着这个类主要的功能是：

* 为export提供一个非模板类型的抽象定义
* 与export registry打交道
* 定义接口函数
...
sc_export_base的两种类型的构造函数，都是把自己注册到systemc kernrl维护的
`export registry` 中去，而析构的时候，则从这个export中挪去。这样一来，所有继承自
sc_export_base的派生类就不必与 `export registry` 打交道，因为基类sc_export_base
已经完成了相应的工作了。

=== sc_export

sc_export是一个模板类，直接继承自sc_export_base:
...
sc_export<IF>模板类拥有的唯一一个成员变量，就是指向 `IF` 的指针 `m_interface_p`。
...
((sc_event_finder))数据结构的主要作用，就是为sc_port作为static sensitive源的时候，解决
