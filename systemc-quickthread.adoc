...
* `get_main` ，返回代码主线程的协程参数（主线程也是一个特殊的协程）

在 `sc_cor_qt.cpp` 中，SystemC使用了quickthread实现了这些接口。

注意，`get_main` 返回的是一个静态全局变量 `main_cor` 的指针：
...
这是因为，主线程作为一个协程，是一种特殊的协程。特殊之处在于，这个协程并不是由quickthread库新建的，而是由系统创立的。
所以main_cor存在的目的，主要是为了记录主线程作为协程切换时，需要切换回来的上下文信息。main_cor只需要初始化
为全零的数据（不初始化也行），然后在第一次，主线程作为一个协程将控制权交给另一个协程的时候，main_cor中就会被qt的
协程切换部分填入主协程（就是主线程）恢复执行时候的信息了。

=== 调用栈的创建和销毁

在基于Quickthreads的SystemC实现中，创建一个协程context的方式如下：
...
sc_cor_pkg_qt::create( std::size_t stack_size, sc_cor_fn* fn, void* arg )
{
    ...
    void* aligned_sp = stack_alloc( &cor->m_stack, &cor->m_stack_size ); <1>
    if( aligned_sp == NULL )
    {
    ...
    cor->m_sp = QUICKTHREADS_SP( aligned_sp, cor->m_stack_size ); <2>
    cor->m_sp = QUICKTHREADS_ARGS( cor->m_sp, arg, cor, (qt_userf_t*) fn,
                                   sc_cor_qt_wrapper ); <2>
    return cor;
}
----
<1> 调用stack_alloc，分配协程运行所需要的栈
<2> 将所需要运行的函数设置到协程栈内

其中，这里使用了stack_alloc函数，用来分配协程所需要的调用栈。stack_alloc的实现如下：
...
从stack_alloc实现可以看出，这个函数主要用来分配协程的调用栈空间，并且需要保持一定的对齐条件。对齐约束
可以使用 `posix_memalign` 直接分配获得，或者使用先分配大的内存空间，然后手动对齐的方式获取。如果支持
最新的C{plus}{plus}17特性，可以使用std::aligned_alloc来达到相同的目的，参考https://en.cppreference.com/w/cpp/memory/c/aligned_alloc
部分的介绍。

---

SystemC的协程栈有一定的深度，为了在栈爆掉的时候，显式抛出错误，而不是默默跑飞，Quickthreads的协程
方案提供了一个栈保护的功能：
...
sc_cor_qt::stack_protect( bool enable )
{
    ...
    if( enable ) {
        ret = mprotect( redzone, pagesize - 1, PROT_NONE ); <1>
    }
...
栈保护功能通过((stack_protect))函数实现。主要思想是，将栈最后一部分空间设置为特定的属性，这样
当用户栈爆掉，踩到这部分保护的空间时，就可以由系统抛出中断，从而提示用户栈空间爆掉了。

---

当协程不再使用的时候，析构函数中，使用 `std::free` 释放了分配出来的协程调用栈空间：
...
此时没有做任何的栈unwinding操作。

注意，SystemC中，调用栈的创建和销毁都是由SystemC内部，通过申请 *堆* 上内存的方式完成的。这意味着，如果在SystemC
thread线程内部，栈上分配的临时变量只会释放占用的空间，而不会触发任何副作用。例如，如果有一个shared_ptr变量，
那么在SystemC退出的时候，这个shared_ptr指向的对象的引用并不会自动减一，因为无法做到自动的释放。这样一来，
SystemC配合智能指针使用的时候，基本上必然伴随着内存泄漏。这些内存泄漏普遍发生在SystemC执行完毕退出的时候，从
这层来看，也是可以忍受的。

[NOTE]
====
在最新的SystemC 2.3.4版本中，SystemC优先使用mmap函数来分配quickthread所需要的栈空间。
====
