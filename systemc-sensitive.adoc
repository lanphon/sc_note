...
的add_static_event成员函数，而对于SC_CTHREAD而言，不能用这种方式设置静态
敏感事件。
...
设置到port中。这个port在elaboration done阶段，被调用的complete_binding函数里：
...
sc_port_base未来所绑定的**所有**的sc_interface的default_event()返回的事件对象，
作为静态敏感对象。

同样，SC_CTHREAD无法使用这种方式设置sc_port_base派生类为静态敏感源。
...
如果将一个sc_event_finder派生类作为敏感源设置进来，那么调用的则是这个函数：
...
sc_event_finder同样无法为SC_CTHREAD设置静态敏感源。
...
其中，前三种重载形式并无特殊之初，而第四种，也就是使用sc_event_finder作为静态敏感源：
...
都无法为SC_CTHREAD设置静态敏感源。从<<usage::sc_module::set_process_handle，这里>>我们也
能看出，无法使用sc_sensitive_pos/sc_sensitive_neg为SC_CTHREAD设置静安源，这是因为sc_cthread_handle
根本就没有保存到sc_sensitive_pos和sc_sensitive_neg的地方。

为了支持SC_CTHREAD的功能，sc_sensitive特意提供了如下几个重载了 `operator()`
的方式，设置静态敏感源。
...
sc_sensitive_pos和sc_sensitive_neg都被废弃掉了，因为这两个类的功能都可以被sc_sensitive
覆盖。
