...
所谓的调度条件，指的是何时Method线程会被再次执行。所以在仿真器运行过程中，一个Method线程会被多次执行。

=== ( C )Thread线程
[[concept::c_thread]]

Thread线程在SystemC中一般用来建模时序逻辑。相比于Method线程，Thread线程的特点是**阻塞式**的，在仿真器运行过程中，每个Thread
线程只会执行一次。一个设计良好的Thread线程应当内部包括一个 `while(true) {}` 的循环体，以保证函数体永远不会返回。
...
对于Thread线程，这转而执行了 `sc_thread_process::wait(const sc_envet&)` 了:
...
* 通过 `sc_signal` 调用了 `sc_event::notify_next_delta()`
...
还没来得及执行，那么这个notification等同于丢失，因为 `notify()` 之后立马调用了应当在
...
替代不带参数的 `notify()` 的形式是 `notify(SC_ZERO_TIME)` ，这在<<phase::evaluate, EVALUATE>>阶段会设定事件为一个DELTA类型的
event，注册到 `simc` 的 `m_delta_events` 成员变量中。等全部的线程都完成一轮调度之后，再
执行此event对应的 `trigger()` ，保证了顺序。
