...
成员函数中使用这个数组下标，调用所有有效指针的 free成员函数，这样一来就自动释放掉了内部的
...
<4> 对于extension部分，深拷贝策略是，对于某个类型的extension，如果拷贝源有，同时拷贝目的对象也有，则调用对象的copy_from
...
<8> update_extensions_from中，只有当拷贝源和拷贝目的对象相同类型的extension都有效，才会将extension从源拷贝到目的中，调用的是
...
----
<1> 唯一的构造函数是私有函数，因此这个类不能被其他部分create出来，其实这是为了保护这个类的单件实现
...
* 一组地址，即DMI区域的起始和结束的地址，m_dmi_start_address和m_dmi_end_address
...
tlm_fw_transport_if是一个表达前向通路（forwarding path）的interface，是TLM 2.0的核心接口。
...
肯定数据已经发送完毕。blocking的API调用也意味着没有反向通路（即tlm_blocking_transport_if不区分FW和BW）。
...
类型信息，默认类是<<api::tlm_generic_payload, tlm_generic_payload>>，定义了一个接口
函数transport_dbg:
...
BW的意思是Backward,也就是从target到initiator的通信，
...
在定义 ((tlm_initiator_socket))之前，先定义一个tlm_base_initiator_socket_b的模板虚基类：
...
而get_protocol_types，其得到的结果是<<api::tlm_base_protocol_types, tlm_base_protocol_types>>
的typeid，这是因为BW_IF::protocol_types的定义默认就是<<api::tlm_base_protocol_types, tlm_base_protocol_types>>
...
而get_base_export，则返回成员变量m_export。这里需要额外注意的一点是，对于get_base_interface，返回的
...
target socket作为一个sc_port，绑定了本initiator的interface（本initiator socket的
...
并不是一个虚函数，所以 tlm_base_initiator_socket 的 `void operator() (base_target_socket_type&)`
也不是虚函数。
...
第二个initiator socket的m_export绑定到了本m_export内部的interface上。注意这里的绑定方向，当
本initiator socket绑定到第二个initiator socket的时候，本initiator socket作为sc_port，需要
从本socket绑定到第二个socket上。而绑定export的时候，需要从第二个socket绑定到本socket上。
...
的时候，本initiator socket作为一个sc_port，并没有参与到绑定过程中。在使用过程中，后续需要将initiator
socket作为一个sc_port绑定上，否则会出现悬空的port。这是因为这里的initiator socket作为一个
继承自sc_port的派生类，使用的策略默认值是至少绑定一个。
...
((tlm_target_socket))的定义方式和tlm_initiator_socket的定义方式基本一样，也是分为tlm_base_target_socket_b
...
一样，与其基类相比，并没有实现更多的功能，设计的初衷主要是为了修改模板类型。tlm_target_socket
...
就需要参与同步了。这样设置的效果是，所有的socket以一个设置的固定的间隔同步，比如这里的9ns。
...
相比于TLM 1.0，TLM 2.0的设计采取了更加复杂的协议。Interface传递的数据可以被下游修改，从而实现数据传递需要的功能。延时时间参数的加入，也使得TLM 2.0
相比TLM 1.0，更加接近transaction level的概念。同时，这也使得TLM 2.0相比于TLM 1.0，摆脱了对primitive channel的束缚。在TLM 1.0中，
...
在TLM 2.0中，SystemC仿真所必须的三要素，Thread和Event仍然存在，然而Primitive Channel的概念已经被弱化，甚至可以完全不需要任何Primitive
channel了。
...
一对前向和后向通路，前向获取DMI数据，后向则invalidate DMI所返回的信息
...
需要强调的是，nonblocking的两个标准API，即tlm_fw_nonblocking_transport_if和tlm_bw_nonblocking_transport_if
的接口函数，nb_transport_fw和nb_transport_bw必须是无阻塞的。这意味着通信的发起方或者接收方调用这两个接口函数的时候，
必须立即返回，而不能在函数的执行过程中显式或者隐式地引入可能的阻塞。例如，在这两个接口函数中，调用sc_fifo的read/write都是不允许的，因为sc_fifo的
read/write操作是可能阻塞的。
...
就能实现一致性的仿真结果，同样极大方便和其他语言之间的沟通。例如，在UVM中同样可以实现TLM，这样C{plus}{plus}侧的代码就可以使用TLM 2.0
协议和SystemVerilog之间相互交换信息，这就是TLM 2.0所强调的互操作性了。
