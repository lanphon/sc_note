...
2.0.6版本。 TLM的组织框图如下所示：

.TLM结构，来源自TLM LRM文档
image::tlm_framework.png[]


本部分主要集中于TLM 2.0.6版本中的interoperability layer（互操作层）部分的介绍。
首先，我们从tlm中的socket概念入手，逐渐探索TLM的实现机制。
...
tlm_mm_interfashice是一个接口基类，只要求实现接口函数 `free` 即可：
...
{}
----
<1> m_extensions的大小又max_num_extensions决定，这个函数返回的是所有注册类型的
总得个数
...
数据。对于 `set_extension`，将数据设置到 m_extensions后，将原来保存的数据返回。
...
相比于 `set_extension` 而言，主要在于，如果原先要被替换的位置费控，那么就将这个位置的指针
...
从这个成员函数的底线可以看出，`deep_copy_from` 主要处理的是两个buffer的深拷贝，以及extension的拷贝策略。
...
tlm_generic_payloa的 `free_all_extensions` 成员函数，主要用来释放所有的extensions。这些extensions
...
，同时接受一个PHASE类型，默认是<<api::tlm_phase, tlm_phase>>，以及一个时间戳t，返回一个tlm_sync_enum信息。
...
接受一个TRANS类型的数据和一个时间戳，返回空。

b_transport的含义是blocking地传送数据，所以不需要返回状态信息，因为blocking地传送数据返回的时候
肯定数据已经发送完毕。同时这里也不需要PHASE信息。
...
继承自两个虚基类，分别是tlm_bw_nonblocking_transport_if和tlm_bw_direct_mem_if:
...
* get_base_port不再返回sc_core::sc_port_base引用，而是返回sc_core_sc_prot_b<FW_IF>引用，后者
...
* FW_IF需要实现<<api::tlm_fw_blocking_transport_if::b_transport, b_transport>>
* FW_IF需要实现<<api::tlm_fw_direct_mem_if::get_direct_mem_ptr, get_direct_mem_ptr>>
* FW_IF需要实现<<api::api::tlm_transport_dbg_if::transport_dbg, transport_dbg>>
...
的过程中，整合了socket的概念。initiator socket是一个sc_port，同时也拥有一个sc_export的成员变量。
target socket是一个sc_export，同时也拥有一个sc_port的成员变量。

initiator socket和target socket互相之间可以绑定，应用有4种绑定关系，再加上initiator socket和
...
* target sockeet绑定到initiator socket，这和initiator socket绑定到target socket效果是一样的
