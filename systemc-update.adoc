...
目前最新的版本是3.0.1，但是之前的版本也可以使用。相比于之前版本，3.0.0，以及后续的3.0.1版本将C++的标准
提升到了c++17，这意味着如果编译器不支持C++17，那么3.0.0版本的SystemC将无法编译和使用。
...
在C++版本提升到C++11之后，我们可以直接使用decltype的方式获取当前类名称。这个重要的改动意味着实际上
我们无需SC_HAS_PROCESS来做多余的一步了。尽管C++11在2011年就已经出现了，但是在SystemC的开发过程中
