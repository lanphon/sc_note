...
而对于simple socket，则在内部实现了继承有<<data::tlm_fw_transport_if, tlm_fw_transport_if>>
和<<data::tlm_bw_transport_if, tlm_bw_transport_if>>的对象，提供了相应接口函数的实现。这些接口函数内部仍然
...
<<bw_process::nb_transport_bw>>中处理，直到fw返回END_RESP的时候，才再次调用这个API：
...
首先，我们先了解下<<api::tlm_sync_enum, tlm_sync_enum>>。
