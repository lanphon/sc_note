...
而在本部分，我们将从simple socket入手，来了解socket的实现细节，并且从这些实现细节触发，来尝试去掌握如何理解TLM 2.0协议的内在约束。
...
((simple_initiator_socket))包括一系列sockets定义，同样有着一套复杂的继承关系：
...
方法的类process，这两个接口函数实际调用了注册的回调函数。同时，simple initiator socket作为一个
...
<2> 这里不会走到返回TLM_COMPLETED，如果走到就意味着系统出现错误
...
target socker而言，其内部需要实现FW_IF的功能。FW_IF有4个接口函数需要实现，同时，还需要考虑到，其中两个接口函数 `nb_transport_fw`
...
中，b_transport的额外延时参数t保证了发起读和返回一定不在同一个delta cycle内（因为至少需要延时
...
准确来说，只能有一个transaction可以处于req发起阶段。当initiator socket发起BEGIN_REQ请求之后，
target回TLM_ACCPETED或者TLM_UPDATED之前，initiator socket不可以发起下一笔transaction。
但在target socket返回任何信息之前，initiator socket都是未阻塞状态，可以执行其他操作，例如为下一笔
transaction做准备。

TLM_COMPLETED约束为，只能在以下场景可能返回：

* 一个target socket从forward path接受到BEGIN_REQ phase
** 这意味着本次传输全部结束，不再需要END_REQ/BEGIN_RESP/END_RESP阶段
...
https://zhuanlan.zhihu.com/p/1900536457220843492