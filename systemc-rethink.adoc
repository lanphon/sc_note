...
event是不可获取的，然而primitive channel却可以有别的方法代替。
...
对于派生自sc_module, sc_port, sc_export和sc_primitive_channel的子类，可以实现以下几种
...
这四个函数在对应的基类中都是虚函数(然而并不是纯虚函数)。派生类如果需要的话，可以overrie这些方法。
