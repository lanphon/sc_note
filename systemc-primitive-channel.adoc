...
(((sc_prim_channel)))
`sc_prim_channel` 是所有channel的基类，继承自<<data::sc_object, sc_object>>。
...
对于((sc_fifo))，如果非阻塞的读或者写成功的话，就会调用 `request_update` 成员函数，将自己
...
sc_clock特化了sc_signal模板，数据类型为布尔型，并且策略只允许`SC_ONE_WRITER`.
...
((sc_semaphore))是一个用来在多个thread之间做同步的信号量。sc_semaphore只考虑在systemc线程之间做同步，如果需要同步的
一方不是systemc线程，那么就需要考虑其他诸如<<data::sc_host_semaphore, sc_host_semaphore>>之类的方法了。
...
public:
    ...
    explicit sc_semaphore( int init_value_ ); <1>
    sc_semaphore( const char* name_, int init_value_ ); <1>
...
protected:
    ...
    sc_event m_free;        // event to block on when m_value is negative <3>
    int      m_value;       // current value of the semaphore
};
----
<1> sc_semaphore的构造函数可以提供一个初始值，表明这个semaphore的并发量
<2> in_use函数表明当前的semaphore是否不可用
<3> 内部实际上使用了一个<<data::sc_event, sc_event>>类型的m_free来控制

sc_semaphore继承自sc_semaphore_if，主要实现了包括wait, trywait和post语义的接口。

=== wait
[[api::sc_semaphore::wait]]

wait是一个阻塞的接口。如果当前semaphore不可用，那么调用方将被阻塞住，直到semaphore
变得可用为止。wait的实现非常简单：
...
实际上wait就是检查当前semaphore的in_use状态。如果sc_semaphore不可用，那么wait将阻塞
住，并等待`m_free`事件的触发。由于sc_semphore可以被多次锁定，因此即使m_free事件触发，我们
仍然需要继续检查in_use状态。

当semaphore处于可用状态后，wait将内部计数器m_value减一，然后返回0. wait的返回值是没有
意义的，所以可以忽略wait的返回值。


=== trywait
[[api::sc_semaphore::trywait]]

与wait相比，trywait是一个非阻塞的调用。调用方将收到一个返回值，表明是否成功获取了
semaphore:
...
这里trywait只是简单检查in_use状态。如果semaphore不可用，那么就返回负一。如果semaphore
处于可用状态，那么就将内部计数器减去一，然后返回零。trywait的返回值是有意义的，调用方必须
检测返回值，来确认自己是否获取了semaphore。

=== post
[[api::sc_semaphore::post]]
...
post简单将内部计数器加一，然后触发事件m_free，来唤醒可能阻塞在m_free上的任何wait操作，之后返回零。

== sc_host_semaphore
[[data::sc_host_semaphore]]

((sc_host_semaphore))是一个用来在多个thread之间做同步的信号量。与<<data::sc_semaphore, sc_semaphore>>相比，
sc_host_semaphore主要突出了host的特点，即可以在systemc的线程和非systemc的host线程之间做同步的能力。
sc_host_semaphore的实现
有几种形式，我们这里只考虑基于C{plus}{plus}11的标准实现方法。
...
之外的事件发生，来和SystemC内部的对象事件等进行交互。SystemC从版本2.3.2开始，正式
支持异步框架。2.3.2版本以前的SystemC，异步框架只是实验性质，并不成熟。
...
* `async_attach_suspending`，用来将一个primitive channel继承类附加到SystemC仿真内核上，这样即使SystemC
内部没有timed event，或者starve的时候，也不会退出，因为async的primitive channel可能会有数据到来
* `async_detach_suspending`，作用和async_attach_suspending相反，将一个primitive channel继承类从SystemC
仿真内核上移除，这样SystemC就不再对这个primitive channel特殊处理

CAUTION: 即使是一个被detach suspending的async primitive channel，SystemC内核也能正常处理这个数据的请求，只不过
在SystemC进入starve状态的时候，不再考虑这些detach suspending的async primitive channel而已。

这三个API都是通过调度registry的同名API来完成的。第一个API是最为广泛使用的，我们这了
主要讨论这个API。 `async_attach_suspending` 和 `async_detach_suspending`
主要和异步阻塞，防止SystemC内核提前结束仿真有关，我们后续会详细介绍具体的工作机制。
...
sc_prim_channel的async_attach_suspending最终调用的就是async_update_list的attach_suspending
请求。sc_prim_channel的async_attach_suspending首先调用sc_prim_channel_registry的async_attach_suspending:
...
而sc_prim_channel_registry的async_attach_suspending调用的则是
async_update_list的attach_suspending:
...
sc_prim_channel的async_detach_suspending最终调用的就是async_update_list的detach_suspending
请求。具体的调用方式和<<data::async_update_list::attach_suspending, attach_suspending>>
的方法一样，这里就不再赘述了。
...
class thread_safe_if : public sc_interface
{
    public:
    virtual void notify(sc_time t = SC_ZERO_TIME) = 0;
    virtual const sc_event& default_event(void) const = 0;
};

class thread_safe_event final 
: public sc_prim_channel, public thread_safe_if
{
    public:
    thread_safe_event(sc_module_name name = "")
    : sc_prim_channel(name)
    {
        // async_attach_suspending();
    }

    void notify(sc_time t = SC_ZERO_TIME) override
    {
        m_delay = t;
        async_request_update(); <1>
    }

    const sc_event& default_event(void) const override { return m_event; } <2>

    void update() override
    {
        // async_detach_suspending();
        m_event.notify(m_delay); <3>
    }
    private:
    sc_event m_event; <4>
    sc_time m_delay;
...
{
    public:
    SC_HAS_PROCESS(m_foo);
    m_foo(sc_module_name name = "")
    : sc_module(name)
    , m_event("async_event")
    {
        SC_METHOD(get_event_method);
        sensitive << m_event; <5>
        dont_initialize();
    ...
    void main()
    {
        while (true) 
        {
            ...
            wait(SC_ZERO_TIME);
            std::cout << "main thread sleep for 3 seconds" << std::endl;
        }
    }

    void get_event_method()
    {
        std::cout << "get async event" << std::endl;
    }
    public:
    thread_safe_event m_event;
};

void async_thread(m_foo* foo)
{
    std::this_thread::sleep_for(std::chrono::seconds(7));
    foo->m_event.notify(SC_ZERO_TIME); <6>
    std::cout << "trigger event from async thread" << std::endl;
}

int sc_main(int argc, char **argv)
{
    m_foo foo("test_module");

    std::thread t0(async_thread, &foo);
    sc_start();
...
上边这个例子使用了一个 `main` 的sc_thread，用来防止sc内核提前退出。如果去掉 `main` 的sc_thread，可以看到
仿真器很快从 sc_start 返回，接下来等待一段时间后，外部打印一个" trigger event from async thread" 后
就退出了。

为了避免这种场景的出现，可以在构造函数的地方调用<<data::async_update_list::attach_suspending, async_attach_suspending>>
函数，并且在update的时候调用 <<data::async_update_list::detach_suspending, async_detach_suspending>>，这种机制
就可以保证外部的thread没触发之前，SC内核不会退出。注意这里只是一个简单的示例，只能应对一次性的使用场景。
实际上async_detach_suspending应当在确保没有更多请求数据的时候被调用。

增加了async_attach_suspending和async_detach_suspending之后，可以看到SC内核收到异步通知，执行了 `sc_method_process`
之后，才退出的。
...
这种类型不过是一个<<data::sc_module, sc_module>> 而已：
