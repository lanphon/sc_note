...
`sc_prim_channel::request_update` 可能被多次调用。

`sc_prim_channel::request_update` 函数应当在EVALUATE PHASE调用。
...
在UPDATE PHASE的工作。这些类可以用 `sc_prim_channel::request_update` ，来和
primitive channel registry通信。
...
这里读和写时间都使用了 `notify(SC_ZERO_TIME)` ，在UPDATE阶段将阻塞在这两个事件的
...
链表中。RT线程执行的时候，由于fifo中已经有写入的数据，所以也不会阻塞，直接读取数据，同样调用
request_update将本fifo放入registry的update链表。在UPDATE阶段， `update` 函数被调用，读和写
事件都有发生，因此会调用读和写事件的 `notify(SC_ZERO_TIME)`  ，尝试将阻塞在读写事件上的线程放入
runnable列表。由于没有线程实际阻塞在读写事件上，因此 `notify(SC_ZERO_TIME)` 实际上没有
为下一个delta cycle调度添加任何线程。在这种情况下，读和写发生在一个clock，准确地说，在同一个delta cycle
内完成。
...
如果EVALUATE PHASE先调度RT，再调度WT。RT和WT都能够顺利完成。在UPDATE阶段，由于没有
阻塞在读和写事件上的线程，因此不会为runnable链表增加任何线程。这种情况下，读和写发生在
一个clock内，并且发生在同一个delta cycle内。
...
`sc_fifo` 令人比较困惑的一点在于，在FIFO为空的情况下，如果读写发生在同一个clock cycle，那么
...
重要的一个功能：打断双向的时延，已提供更好的timing能力。
...
是为了让sc_signal有着不同的逾期行为：是否可以被多个线程同时赋值（MANY WRITER），是否
